...
CapacitiveSensor c_2 = CapacitiveSensor(9, 11);
...
int calibration = 0;
int ndelay = 0;
int ledStatus = 0, ledStatus_2 = 0;
int mode = 0;
String code = "";
int randNumber = 0;
int ligado = LOW, ligado_2 = LOW;
unsigned long csSum, csSum_2;
...
void CSread_2() {
  long cs;
  if (calibration != 0) {
    cs = c_2.capacitiveSensor(calibration);
  } else {
    cs = c_2.capacitiveSensor(30); //a: Sensor resolution is set to 80
  }
  if (cs > 30) { //b: Arbitrary number
    csSum_2 += cs;
    int dl;
    ...
    //Serial.println(cs);
    if (csSum_2 >= dl) //c: This value is the threshold, a High value means it takes longer to trigger
    {
      ff_2 = 1;
      //Serial.print("Trigger: ");
      //Serial.println(csSum);
      if (csSum_2 > 0) {
        csSum_2 = 0;
        if (ligado_2 == LOW && ff_2 == 1) {
          ligado_2 = HIGH;
          ledStatus_2 = HIGH;
          ff_2 = 0;
          digitalWrite(A2, ledStatus_2);
        }
        if (ligado_2 == HIGH && ff_2 == 1) {
          ligado_2 = LOW;
          ff_2 = 0;
          ledStatus_2 = LOW;
          digitalWrite(A2, ledStatus_2);
...
void getAddr(char id) {
  ...
  //Serial.println(randNumber);
   closeRespCom(id, ip + "+" + String(randNumber)+ "+" + String(2));
  mode = 0;
}
...
void oneCom() {
  ...
  index = response.indexOf("/H1" + code);
  if (index != -1) {
    ledStatus = 1;
    ff = 0;
    digitalWrite(A1, ledStatus);
  ...
  index = response.indexOf("/L1" + code);
  if (index != -1) {
    ff = 0;
    ledStatus = 0;
    digitalWrite(A1, ledStatus);
  ...
  index = response.indexOf("/H2" + code);
  if (index != -1) {
    ledStatus_2 = 1;
    ff_2 = 0;
    digitalWrite(A2, ledStatus_2);
  ...
  index = response.indexOf("/L2" + code);
  if (index != -1) {
    ff_2 = 0;
    ledStatus_2 = 0;
    digitalWrite(A2, ledStatus_2);
  ...
  index = response.indexOf("/o1" + code);
  if (index != -1) {
    out = 1;
  }
  index = response.indexOf("/o2" + code);
  if (index != -1) {
    out = 2;
  }
  index = response.indexOf("/o3" + code);
  if (index != -1) {
    out = 3;
  }
